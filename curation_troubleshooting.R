...
dt_curated <- curate_dead_animals(dt, prop_immobile = 0.0001, resolution = 96)
summary(dt_curated)
...
#### Metadata for manual curation of dead animals: 2566
...
manual_metadata <- data.table(file = rep(c("Data/EF_01032023_2566/Monitor77.txt",
                                    "Data/EF_01032023_2566/Monitor78.txt"), each = 32),
                       ## a unique experiment ID if you want to analyse across experiments
                       ...
                       ##startdatetime at lights on day before 
                       start_datetime = 
                         c(rep("2023-02-21 03:00:00", times = 64)),   #ZT0                  
                       ## experimental stop time
                       stop_datetime = 
                         c(rep("2023-02-28 03:00:00", times = 64)),
                       ## this is the tube number position in each monitor
                       region_id = rep(1:32, 2),
                       ## the genotypes used in the experiment
                       genotype = rep(c("2566",
...
#add in which mosquitoes were alive
manual_metadata[c(21,22,24,27,29,31,50,52:63), status := "alive"]
...
###### Metadata for automatic curation of dead animals: 2566

metadata <- data.table(file = rep(c("Data/EF_01032023_2566/Monitor77.txt",
                                    "Data/EF_01032023_2566/Monitor78.txt"), each = 32),
                       ## a unique experiment ID if you want to analyse across experiments
                       exp_ID = rep(c("AGAP002566_GFP"), each = 64),
                       ## a link to the environmental monitor file use in the experiment
...
lifespan_dt <- dt_curated[, .(lifespan = max(t)), by=id]
# we filter this table for lifespan>2 and we keep the id
valid_ids <- lifespan_dt[lifespan > days(5.5), id]
# we apply this filter
dt_curated <- dt_curated[id %in% valid_ids]
summary(dt_curated)
